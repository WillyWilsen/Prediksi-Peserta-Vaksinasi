...
library(shinythemes)
  ...
  ui <- fluidPage(theme = shinytheme("cerulean"),
    navbarPage(
      "Prediksi Peserta Vaksin",
      tabPanel("Home",
               sidebarPanel(
                 tags$h3("Upload csv file:"),
                 fileInput("file", ""),
               ),
               mainPanel(
                            h1("Data Prediksi"),
                            tableOutput("tabel_prediksi"),
                            plotOutput("data_prediksi"),
                            h1("Ketepatan"),
                            tableOutput("galat")
               )
      )
  ...
  server <- function(input, output) {
    dataset <- reactive({
      if (is.null(input$file)) {
        ...
        filespec <- input$file
        datauji <- read.csv(filespec$datapath, TRUE, ";")
        data = datauji$Jumlah
        return(data)
      }
    })
    ...
    ketepatan <- reactive({
      if (is.null(dataset())) {
        ...
        data = dataset()
        data_used = data[c(2:length(data))]
        error = abs(data_used-peramalan())
        MAPE = mean(abs(error/data_used*100), na.rm=TRUE)
        hasil = cbind(MAPE)
        return(hasil)
      }
    })
    
    output$tabel_prediksi <- renderTable({
      if (is.null(dataset())) {
        return ()
      } else {
        tabel_out()
    ...
    output$data_prediksi <- renderPlot({
      if (is.null(dataset())) {
        return ()
      } else {
        plot(dataset(),
             xlab="Periode",
             ylab="Jumlah Vaksinasi",
             type = "l")
        lines(peramalan(),
              col = "red")
        labels <- c("Awal", "Prediksi")
